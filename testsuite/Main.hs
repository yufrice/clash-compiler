...
import qualified Examples.ALU as Examples.ALU
...
runClashTest =
  defaultMain $ clashTestRoot
    [ clashTestGroup "Examples"
      [  runTest 'Examples.ALU.topEntity def{hdlSim=False,hdlDirs=Just [""]}
...
main = do
  ...
  -- TODO: Use Nix, relying on ghc environment files is just horrible
  -- Get Clash binary
  ...
  -- Make sure all tests are available
  let cp = proc "cabal" ["--write-ghc-environment-files=always", "-v2", "new-build", "testsuite"]
